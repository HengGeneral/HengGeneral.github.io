...
```
    ...
    /*
     * 从该方法可以看到, Hashtable中key和value不能为null
     */
...
HashMap也用hash-based table 来存储key-value的。和Hashtable不同的是, HashMap 允许key和value
为null; 还有一个比较重要的是, HashMap并不是synchronized的, 因此当多个线程并发地访问hashmap
且至少有一个线程会变更hashmap的结构时, 需要在外部进行同步控制或者使用 Collections.synchronizedMap()
进行包装。
            ...
            for (int j = 0; j < oldCap; ++j) {
                ...
                if ((e = oldTab[j]) != null) {
                    ...
                    if (e.next == null)
                        ...
                        do {
                            next = e.next;
                            // e.hash & oldCap其实做的事情就是判断resize后放入的槽是新的还是旧的
                            // 如果是旧的则保存之前的顺序放入对应的"旧"槽; 否则放到"新"槽
                            if ((e.hash & oldCap) == 0) {
                                if (loTail == null)
                                    loHead = e;
                                else
...
```
该算法返回大于等于当前cap值（int类型）的最小2的power算法。思路则是，对正数来说，从最高位位1的那个位开始，
后面一直置为1，结果加1后得到的结果则是大于该数的最小2的power。但如果cap的值刚好是2的倍数，返回的结果则刚好是cap的2倍。
比较好的方式是，先对cap减1操作进行预处理，后续就直接对高位1开始所有位置位1，最后结果加1即可。

如何用最简单的方式（复杂的可以用循环语句）对高位后的各位都置为1呢？ 将该值无符号右移1位，并与该值异或，能得到高位1和次高位1的结果，并对该值更新，
那么当前值就高位和次高位都为1了（至少2个连续位为1）；然后将变化后的值无符号右移2位，并异或，等到4个连续位为1了；以此类推到移动16位（1+2+4+8+16=31），
就能保证该int值高位后连续都为1的情况了。若cap不是int型，为long、byte等型，移动的位数简单调整即可。
...
将高16位值和低16位进行异或放在低16位，原高16位保留。
然后和散列的范围indexFor方法进行取模得到最终的散列值。

已有[OPTIMISING_HASHING][数据表明]，该方法可以有效地减少hash碰撞。
...
[OPTIMISING_HASHING]: http://vanillajava.blogspot.com/2015/09/an-introduction-to-optimising-hashing.html
